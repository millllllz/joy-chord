...
**Current build:** v33 (bottom-left corner of the app; bump on every deploy, see Conventions)
...
**Effects**: a shared lowpass filter (BiquadFilterNode, default 2.5kHz cutoff/Q 1), delay (DelayNode + feedback gain, defaults 280ms/32%/22% send), and algorithmic reverb (ConvolverNode with a synthesized impulse response — no external audio file, defaults 2.2s decay/18% send) are built lazily in `init()` (`src/effects.js`), one node/bus each shared by every voice. The "Filter"/"Delay"/"Reverb" buttons (in the center control column, alongside the key and waveform dropdowns) each open a `<dialog>` with a checkbox and a range slider per parameter — filter gets cutoff/resonance, delay gets time/feedback/send, reverb gets decay/send. Every slider updates live on `input` except reverb decay, which only commits on `change` since it regenerates the impulse response buffer. The filter's cutoff slider maps its 0-1 travel to frequency exponentially (`sliderToCutoff()` in `src/index.js`), not linearly, since pitch is perceived logarithmically. All three effects can be on, off, or any combination, independent of each other and of their own parameter values.

**Signal flow**: each voice's gain connects only to the shared filter (`effects.filterNode`), not straight to `destination` — the filter fans out to `destination` plus a parallel tap into each effect send, so filtering lands before the delay/reverb taps and the echoes/tail come out filtered too. The delay's output additionally feeds the reverb send as well as `destination` — so the echoes are reverberated rather than dry. That last edge matters more than it looks: without it the repeats are completely dry, which passes unnoticed at short delay times (they land while the original note's reverb tail is still ringing) but makes the space audibly drop out at long ones, since by then the tail is ~27dB down. Nothing downstream of the reverb returns to the delay, so there's no runaway path. `src/test-setup.js`'s mock node records its `connect()` calls so routing like this is assertable — state-only tests can't see a misrouted graph.

**Filter on/off is a ramp, not a bypass**: delay and reverb are parallel sends with an always-live dry path underneath, so "off" just ramps their send gain to 0. The filter is an insert — every voice routes through it — so there's no separate dry path to fall back to; instead "off" ramps `frequency`/`Q` to fully-open values (20kHz / Q≈0) over the same 50ms `FX_RAMP` window used elsewhere, leaving the node in the graph but audibly transparent. Re-enabling ramps back to the configured cutoff/resonance, not a fresh default.
...
- **Tremolo effect** — alongside existing filter/delay/reverb (see Architecture); also open-ended: survey what other effects would fit the HiChord-style signal chain.
