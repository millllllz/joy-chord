...
## Build version SHA
before every push, update all three to the short SHA of the commit about to be made:
- `#build-version` text in index.html
- `?v=` query param on the `src/index.js` script tag in index.html
- `CACHE_NAME` in sw.js (`joychord-<sha>`) — this is what actually busts the service worker cache on iOS Safari / PWA installs
